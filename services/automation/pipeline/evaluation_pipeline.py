#!/usr/bin/env python3
"""
Orchestrates the post-game evaluation workflow:

1. (Optional) scrape Sports Reference finals for the target dates and write them
   back to Firestore so each game doc has `home_score/away_score`.
2. Run `automation.evaluation.evaluate_predictions` across the same date window
   to grade every prediction and publish summary metrics.

Edit `DEFAULT_EVAL_CONFIG` below (start/end/single date) when you want to run a
different slice; start/end take precedence over single_date, and the fallback is
to evaluate yesterday's slate.
"""
